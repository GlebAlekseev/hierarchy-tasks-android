...
class TaskListRepositoryImpl @Inject constructor(private val taskListDao: TaskDao, private val mapper: Mapper<TaskItem,TaskDbModel>): TaskListRepository {
    init {
        initDatabase()
    }
    ...
    private fun initDatabase() {
        GlobalScope.launch{
            ...
            if (taskListDao.getAll().value?.isEmpty() == true){
    ...
    override fun getTaskItem(taskItemId: Long): TaskItem {
        return mapper.mapDbModelToItem(taskListDao.get(taskItemId))
    }

    override fun getTaskList(): LiveData<List<TaskItem>> = Transformations.map(taskListDao.getAll()){
        it.map { item->
